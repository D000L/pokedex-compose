...
interface SearchDao {
  ...
  suspend fun searchPokemonNames(query: String = "", limit: Int = -1): List<IndexedItem>

  @Query("SELECT id, name FROM pokemon_move WHERE name LIKE '%' || :query || '%' ORDER BY id ASC LIMIT :limit")
  suspend fun searchMoveNames(query: String = "", limit: Int = -1): List<IndexedItem>

  @Query("SELECT id, name FROM item WHERE name LIKE '%' || :query || '%' ORDER BY id ASC LIMIT :limit")
  suspend fun searchItemNames(query: String = "", limit: Int = -1): List<IndexedItem>
}